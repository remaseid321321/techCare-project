...
if (tableBody) {
    ...
    for (var i = 0; i < services.length; i++) {
        ...
        html += "<tr>" +
            ...
            "<td>" + services[i].servicePrice + "</td>" +
            "<td>" + services[i].serviceDesc + "</td>" +
            "<td style='text-align:right;'>" +
                "<button class='editBtn' id='edit" + i + "'>Edit</button> " +
                "<button class='deleteBtn' id='delete" + i + "'>Delete</button>" +
            "</td>" +
        "</tr>";
    }
...
for (var i = 0; i < deleteBtns.length; i++) {
    ...
    deleteBtns[i].onclick = function () {
        ...
        var btnId = this.id;        // delete3
        var index = btnId.replace("delete", "");  

        var conf = confirm("Delete this service?");
        if (!conf) { return; }
...
for (var i = 0; i < editBtns.length; i++) {
    ...
    editBtns[i].onclick = function () {
        ...
        var btnId = this.id;
        var index = btnId.replace("edit", "");

        var name = prompt("Edit service name:", services[index].serviceName);
        if (name === null || name === "") { return; }

        var price = prompt("Edit service price:", services[index].servicePrice);
        if (price === null || price === "") { return; }

        var desc = prompt("Edit description:", services[index].serviceDesc);
        if (desc === null || desc === "") { return; }

        services[index].serviceName = name;
        services[index].servicePrice = price;
        services[index].serviceDesc = desc;
